...
Lambda function does the below
    Reads the Alarms from 'AWSIotCoreMonitoring' item available in DDB - AccountFeatureDefinition
    And creates CW alarms for AWS Iot Core service
'''
    ...
    try:
        ...
        if (event['RequestType'] in ['Create','Update']) and ('ServiceToken' in event):
            try:
                ...
                dynamodb_response = dynamodb_client.get_item(TableName='AccountFeatureDefinitions',Key={"Feature":{'S':"AWSIotCoreMonitoring"}}, ConsistentRead=True)
                ...
                if report:
                    text="AWS Iot Core Monitoring, create CW alarms report:\n\n"
                    text+="Sucess: "+str(success_count)+"\nFailed: "+str(failure_count)+"\n\nReport Information:\n\n"
                    print("Alarms creation report - ",report)
                    subject= 'DXCMS AWS Iot Core Monitoring, CW Alarms report. acc-'+ str(acc_id) + ' region ' + region
