...
Lambda function does the below
    Reads the Alarms from 'AWSIotSitewiseMonitoring' item available in DDB - AccountFeatureDefinition
    And creates CW alarms for AWS Iot Sitewise service
    NOTE: Currently, AWS Iot Sitewise is supported in us-east-1,us-east-2,us-west-2,ap-south-1,ap-northeast-2,ap-southeast-1,ap-southeast-2,ap-northeast-1,ca-central-1,eu-central-1,eu-west-1, so this Feature/Offerings should be deployed ONLY in these regions
'''
...
supported_regions=os.environ["SUPPORTED_REGIONS"].split(",")
logging_filter=os.environ["LOGGINGFILTER"]

config = Config(retries=dict(max_attempts=10,mode='standard'))
cloudwatch_client = boto3.client("cloudwatch",config=config)
dynamodb_client = boto3.client("dynamodb",config=config)
sns_client= boto3.client("sns",config=config)
iot_sitewise_client=boto3.client('iotsitewise',config=config)
...
def enable_logging_option(logging_filter):
    try:
        if iot_sitewise_client.describe_logging_options()['loggingOptions']['level']!=logging_filter:
            iot_sitewise_client.put_logging_options(loggingOptions={'level': logging_filter})
            print("Successfully updated iot-sitewise logging option with verocity ",logging_filter)
    except Exception as e:
        print("Error-enable_logging_option",e)
    ...
    try:
        ...
        if (event['RequestType'] in ['Create','Update']) and ('ServiceToken' in event):
            try:
                ...
                dynamodb_response = dynamodb_client.get_item(TableName='AccountFeatureDefinitions',Key={"Feature":{'S':"AWSIotSitewiseMonitoring"}}, ConsistentRead=True)
                ...
                for alarm in alarms:
                    try:
                        ...
                        alarm_Dimensions=""
                        for dim in sorted(dimlist,reverse=True): alarm_Dimensions='-'.join([alarm_Dimensions,dimlist[dim]])
                        alarm_Dimensions=alarm_Dimensions[1:]
                        alarm_name='-'.join(["dxcms",alarm_name_space,alarms[alarm]['MetricName'],alarm_Dimensions])
                        
                        if(region not in supported_regions):
                            raise Exception('Invalid region - {}, please deploy DXC Managed Services in one of these regions {} to create alarms'.format(region,supported_regions))
                        ...
                        enable_logging_option(logging_filter)
                    except Exception as e:
                        print("Error-",e)
                ...
                if report:
                    text="AWS Iot Sitewise Monitoring, create CW alarms report:\n\n"
                    text+="Sucess: "+str(success_count)+"\nFailed: "+str(failure_count)+"\n\nReport Information:\n\n"
                    print("Alarms creation report - ",report)
                    subject= 'DXCMS AWS Iot Sitewise Monitoring, CW Alarms report. acc-'+ str(acc_id) + ' region ' + region
