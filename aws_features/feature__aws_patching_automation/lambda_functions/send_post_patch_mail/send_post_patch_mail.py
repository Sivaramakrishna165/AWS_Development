'''
This Lambda script is used to send post patching mail with health check reports and patching status report
'''
...
def create_HTML_Body(tagValue,patchJobId,patch_type):
    try:
        ...
        BODY_HTML = """	<html>
                ...
                <body>
                <p>Hi Team,</p>
                <p style = "font-weight: bold;font-size: 20px;">"""+patch_type+""" POST PATCHING REPORTS [ HEALTH CHECK & PATCH COMPLIANCE ]</style></p>
                <p>     Patching Activity is completed for the attached servers.</p>
                <p>     Kindly validate Health Check Reports for Windows & Linux which contains Patch Installation Status as well. Also verify attached Post Patch Scan compliance report.</p>
                <p>     """
...
def list_S3_Bucket_object(S3_Bucket,S3_directory_name,tagValue):
    try:
        ...
        prefixs = [S3_Folder_Name + "/" + "PATCHING/" + S3_directory_name + "/HealthCheckOutupts/" + tagValue + "/validation/", S3_Folder_Name + "/" + "PATCHING/" + S3_directory_name + "/PatchScanReports/Post/Post_PatchScanReport_" + tagValue + ".csv", S3_Folder_Name + "/" + "PATCHING/" + S3_directory_name + "/AlarmsAction_enable/Alarms_Action_enable_" + tagValue + "_BY_AY.csv" ]
        S3_Objects = []
...
def read_config_to_send_email(bucket,S3_directory_name,tagValue,patch_type):
    try:
        ...
        accoundId,BODY_HTML = create_HTML_Body(tagValue,patchJobId,patch_type)
        account_name = fetch_account_name()
        BODY_TEXT = "Hi Team, \r\n  Patching has been completed for the Instances. \r\nKindly find the attached reports for the same for verification. Thanks!,\r\n Regards,\r\n CloudOps Automation"
        '''BODY_HTML = """\
        <html>
        <head></head>
        <body>
        <p>Hi Team,</p>
        <p>     Patching has been completed for the Instances.</p>
        <p>     Kindly find the attached reports for the same for verification. Thanks!</p><br>
        <p>Regards,</p>
        <p>CloudOps Automation</p>
        </body>
        </html>
        ...
        SUBJECT = "AWS " + account_name +" "+ patch_type + " PE2EA | Post Patching Reports - " + tagValue     
...
def lambda_handler(event, context):
    ...
    global HealthCheckFullFileName
    global region
    global patchJobId
    global patchMonth
...
if __name__ == "__main__":
    #event1 = {"PatchInstallOn": "DEV-JUN_20_2021_14_0_5HRS","S3_Bucket": "dxc","S3_directory_name": "JUN_2021/ap-south-1/patchJobId_98d80322-c0a9-11eb-b95e-5f544c2b0dea",'Patch_Phase':"pre-patch"}   
    event1 = {"S3_Bucket": "dxc",
