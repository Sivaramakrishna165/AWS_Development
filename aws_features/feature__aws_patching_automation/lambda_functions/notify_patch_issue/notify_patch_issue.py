'''
This lambda script is used to notify user about issue on while patching the instances using SNS
'''
...
def email_body_content(status,Region,Account_id,instanceIds,command_id,failure_cause):
    try:
        print("Region ==> ",Region)
        Account = Account_id
        ...
        CHARSET = "utf-8"
        SUBJECT = "ERROR - AWS " + Account_id + " PE2EA | Patching Failed for Instance : " + instanceIds
        ...
        table = table + "<tr><td>Failure Cause </td><td>" + str(failure_cause) + "</td></tr>"

        BODY_HTML = """ <html>
            <head><style>table, th, td { border: 1px solid black;}</style></head>
            <body>
            <p style = "font-weight: bold;font-size: 20px;color:red;">PATCHING FAILED FOR INSTANCE : """ + str(instanceIds) + """ </style></p>
            <p>Hi Team,</p>
            <p>Patching is FAILED for the Instance - """ + instanceIds + """ . Kindly look into it.</p>"""
...
def read_CW_logs(command_id,instanceId):
    try:
        CW_client = boto3.client('logs',config=config)
        log_streams_response = CW_client.describe_log_streams(    
            logGroupName='/aws/ssm/AWS-RunPatchBaseline',
            logStreamNamePrefix=command_id + "/" + str(instanceId)
        )
        print(log_streams_response['logStreams'][0])
        log_stream = log_streams_response['logStreams'][0]['logStreamName']
        print("LOG_STREAM_ARN : ",log_stream)
        ...
        error_messages = [sub['message'] for sub in events]
        print("error_messages : ",error_messages)
        error_cause = "".join(error_messages)
        print("error_cause : ",error_cause,type(error_cause))
        return error_cause
    except:
        print(PrintException())
...
def lambda_handler(event, context):
    try:
        ...
        failure_cause = read_CW_logs(command_id,instanceIds)
        email_body_content(status,Region,Account_id,instanceIds,command_id,failure_cause)
