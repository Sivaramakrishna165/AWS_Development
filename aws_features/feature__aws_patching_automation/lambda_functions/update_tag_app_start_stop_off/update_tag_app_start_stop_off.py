'''
This Lambda script is used to disable the functionallity of stop/start apps
'''
...
def update_patch_tag_on_instnace(S3_Bucket,S3_directory_name,tagValue,Patching_tag):
    try:
        print("Appending _AY on PatchInstallOn tag...")
        instance_ids = fetch_instance_ids(tagValue,Patching_tag)
        client = boto3.client('ec2',region_name = region,config=config)
        updatedTag = tagValue + "_AY"
        ...
        print("Updated tag on all instances")
    except:
        print(PrintException())
...
def lambda_handler(event,context):
    ...
    event['PatchInstallOn'] = tagValue + "_AY"
    # event1['S3_Bucket'] = S3_Bucket
    # event1['S3_directory_name'] = S3_directory_name
    return event
