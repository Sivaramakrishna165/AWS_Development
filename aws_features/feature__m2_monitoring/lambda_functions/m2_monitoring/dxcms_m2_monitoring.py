...
Lambda function does the below
    Reads the Alarms from 'M2Monitoring' item available in DDB - AccountFeatureDefinition
    And creates CW alarms to monitor M2 (Mainframe Modernization)
'''
...
print("dim_invalid_values are ", dim_invalid_values)
...
def handler(event, context):
    try:
        ...
        if (event['RequestType'] in ['Create','Update']) and ('ServiceToken' in event):
            try:
                #Fetching Alarm details from DynamoDB.
                dynamodb_response = dynamodb_client.get_item(TableName='AccountFeatureDefinitions',Key={"Feature":{'S':"M2Monitoring"}}, ConsistentRead=True)
                parsed_item = ddb_json.loads(dynamodb_response)
                alarms = parsed_item["Item"]["Alarms"]
                print("alarms in handler is ", alarms)
                report={}
                success_count=0
                failure_count=0
                ...
                for alarm in alarms:
                    try:
                        print("alarm in lambda_hanlder is ", alarm)
                        create_alarm_result = None
                        # Ignoring alarm_check in order to update alarm values during stack updates
                        #bln_alarm_check_result = False
                        bln_alarm_check_result = True
                        bln_valid_dim = True
                        alarm_params = {}
                        alarm_name_space=alarms[alarm]['Namespace'].lower().replace("/","")
                        print("alarm_name_space is ", alarm_name_space)
                        dimlist={}
                        for dim in alarms[alarm]['Dimensions']:
                            if dim['Value'].upper() in dim_invalid_values:
                                bln_valid_dim = False
                            dimlist[dim['Name']]=dim['Value']
                        print("dimlist is ", dimlist)
                        if('EnvironmentId' in dimlist):
                            alarm_Dimensions=dimlist['EnvironmentId']
                        
                        alarm_name='-'.join(["dxcms",alarm_name_space,alarms[alarm]['MetricName'],alarm_Dimensions])
                        print("alarm_name is ", alarm_name)
                ...
                if report:
                    text="M2 Monitoring, create CW alarms report:\n\n"
                    text+="Sucess: "+str(success_count)+"\nFailed: "+str(failure_count)+"\n\nReport Information:\n\n"
                    print("Alarms creation report - ",report)
                    subject= 'DXCMS M2 Monitoring, CW Alarms CREATE report. account - '+ str(acc_id) + ' region ' + region
