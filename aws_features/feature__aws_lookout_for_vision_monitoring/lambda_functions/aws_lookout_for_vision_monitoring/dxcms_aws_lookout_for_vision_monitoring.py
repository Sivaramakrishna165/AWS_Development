...
Lambda function does the below
    Reads the Alarms from 'AWSLookoutForVisionMonitoring' item available in DDB - AccountFeatureDefinition
    And creates CW alarms for AWS lookout for vision service
    NOTE: Currently, AWS lookout for vision is supported in us-east-1,us-east-2,us-west-2,ap-northeast-2,ap-northeast-1,eu-central-1,eu-west-1, so this Feature/Offerings should be deployed ONLY in these regions
'''
    ...
    try:
        ...
        if (event['RequestType'] in ['Create','Update']) and ('ServiceToken' in event):
            try:
                ...
                dynamodb_response = dynamodb_client.get_item(TableName='AccountFeatureDefinitions',Key={"Feature":{'S':"AWSLookoutForVisionMonitoring"}}, ConsistentRead=True)
                ...
                if report:
                    text="AWS Lookout For Vision Monitoring, create CW alarms report:\n\n"
                    text+="Sucess: "+str(success_count)+"\nFailed: "+str(failure_count)+"\n\nReport Information:\n\n"
                    print("Alarms creation report - ",report)
                    subject= 'DXCMS AWS Lookout For Vision Monitoring, CW Alarms report. acc-'+ str(acc_id) + ' region ' + region
