...
Lambda function does the below
    Reads the Alarms from 'AWSQuickSightMonitoring' item available in DDB - AccountFeatureDefinition
    And creates CW alarms for QuickSight service
'''
    ...
    try:
        ...
        if (event['RequestType'] in ['Create','Update']) and ('ServiceToken' in event):
            try:
                ...
                dynamodb_response = dynamodb_client.get_item(TableName='AccountFeatureDefinitions',Key={"Feature":{'S':"AWSQuickSightMonitoring"}}, ConsistentRead=True)
                ...
                for alarm in alarms:
                    try:
                        ...
                        for dim in alarms[alarm]['Dimensions']:
                            ...
                            alarm_Dimensions = '-'.join([alarm_Dimensions, dim['Value']])
                        
                        # if('DashboardId' in dimlist):
                        #     alarm_Dimensions=dimlist['DashboardId']
                        # if('DatasetId' in dimlist):
                        #     alarm_Dimensions=dimlist['DatasetId']
                        # if('SheetId' in dimlist):
                        #     alarm_Dimensions = '-'.join([alarm_Dimensions, dimlist['SheetId']])

                        # if('VisualId' in dimlist):
                        #     alarm_Dimensions = '-'.join([alarm_Dimensions, dimlist['VisualId']])
                ...
                if report:
                    text="AWS Quicksight Monitoring, create CW alarms report:\n\n"
                    text+="Sucess: "+str(success_count)+"\nFailed: "+str(failure_count)+"\n\nReport Information:\n\n"
                    print("Alarms creation report - ",report)
                    subject= 'DXCMS AWS Quicksight Monitoring, CW Alarms CREATE report. account - '+ str(acc_id) + ' region ' + region
