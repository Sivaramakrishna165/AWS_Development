"""
    ...
    In dxcms_sh_bf_sfn_diagnosis state machine(CheckTags)
    gets executed after dxcms_sh_sfn_self_heal_master_sfn statemachine.
    In dxcms_sh_bf_sfn_resolution state machine(TagsRemediation)
    gets executed after dxcms_sh_bf_sfn_diagnosis statemachine.
    ...
    On successful check, next state - CheckIAM is called.
    On FAILURE, next State CheckTagsError and then NotifyForLambaFunctionFailure.
    In Resolution state machine, 
    On successful check, next state - IAMRoleRemediation is called
    On FAILURE, next State TagsRemediationError and then NotifyForLambaFunctionFailure.
...
event1 = {"instance_id":"i-03f2a963369545021", "Flag": "Fix"}
