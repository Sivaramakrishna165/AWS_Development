...
def lambda_handler(event, context):
    ...
    try:
        ...
        try:    
            ...
            error = traceback.format_exc()
            raise Exception(f"Error while reading AccountId. Incident in Service Now is NOT created for {instance_id} backup failure issue.")
        
        arn_prefix = "arn:aws:states:" + region + ":" + AccountId + ":stateMachine:"

        short_message = f"AWS_SH_BF {AccountId} {region} | level 2 backup for  {instance_id} does not exist in last 1440 mins"

        long_message = f"SelfHeal Backup Failure Step Function caught error. Backup Failure issue not resolved. Details : {str(event)}."
        ...
        if error:
            raise Exception(f"Error while triggering dxcms_sh_sfn_notification step function. Incident in Service Now is NOT created for {instance_id} backup failure issue.")
