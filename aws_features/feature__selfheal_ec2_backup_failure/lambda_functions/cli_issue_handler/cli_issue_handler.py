...
In dxcms_sh_bf_sfn_diagnosis state machine(CheckCLI),
gets executed after checkSSM.
In dxcms_sh_bf_sfn_resolution state machine(CLIRemediation),
gets executed after wait state of start instance.
...
On FAILURE, next State CheckCLIError and then NotifyForLambaFunctionFailure.
In resolution state machine,
On successful check, next state - choicestatetostopinstance is called.
On FAILURE, next State CLIRemediationError and then NotifyForLambaFunctionFailure.
...
def lambda_handler(event,context):
    ...
    if event["SSM_Ping_Status"] == 'Online':
                        ...
                        if str(script) != str(-1):
                            ...
                            if not error_status:
                                ...
                                if not error_status:
                                    ...
                                    input["error"] = "Error command_id_check() - Error while checking status of command id."
                                    return failure_token(task_token, input, error_status)
                            else:
        ...
        if not error_status:    
            ...
            input["error"] = "Error lambda handler()"
            return failure_token(task_token, input, error_status)
    else:
