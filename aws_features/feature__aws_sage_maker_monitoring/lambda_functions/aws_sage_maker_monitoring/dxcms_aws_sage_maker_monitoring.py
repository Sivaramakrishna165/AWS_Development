...
Lambda function does the below
    Reads the Alarms from 'AWSSageMakerMonitoring' item available in DDB - AccountFeatureDefinition
    And creates CW alarms for AWS Sage Maker service
'''
    ...
    try:
        ...
        if (event['RequestType'] in ['Create','Update']) and ('ServiceToken' in event):
            try:
                ...
                dynamodb_response = dynamodb_client.get_item(TableName='AccountFeatureDefinitions',Key={"Feature":{'S':"AWSSageMakerMonitoring"}}, ConsistentRead=True)
                ...
                if report:
                    text="AWS Sage Maker Monitoring, create CW alarms report:\n\n"
                    text+="Sucess: "+str(success_count)+"\nFailed: "+str(failure_count)+"\n\nReport Information:\n\n"
                    print("Alarms creation report - ",report)
                    subject= 'DXCMS AWS Sage Maker Monitoring, CW Alarms report. acc-'+ str(acc_id) + ' region ' + region
