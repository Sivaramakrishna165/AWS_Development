...
Lambda function does the below
    Reads the Alarms from 'AWSCognitoMonitoring' item available in DDB - AccountFeatureDefinition
    And creates CW alarms for Cognito service
'''
    ...
    try:
        ...
        if (event['RequestType'] in ['Create','Update']) and ('ServiceToken' in event):
            try:
                ...
                dynamodb_response = dynamodb_client.get_item(TableName='AccountFeatureDefinitions',Key={"Feature":{'S':"AWSCognitoMonitoring"}}, ConsistentRead=True)
                ...
                if report:
                    text="AWS Cognito Monitoring, create CW alarms report:\n\n"
                    text+="Sucess: "+str(success_count)+"\nFailed: "+str(failure_count)+"\n\nReport Information:\n\n"
                    print("Alarms creation report - ",report)
                    subject= 'DXCMS AWS Cognito Monitoring, CW Alarms CREATE report. account - '+ str(acc_id) + ' region ' + region
