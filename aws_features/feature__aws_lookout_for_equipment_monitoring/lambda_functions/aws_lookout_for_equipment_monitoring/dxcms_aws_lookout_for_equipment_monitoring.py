...
Lambda function does the below
    Reads the Alarms from 'AWSLookoutForEquipmentMonitoring' item available in DDB - AccountFeatureDefinition
    And creates CW alarms for AWS lookout for equipment service
    NOTE: Currently, AWS lookout for equipment is supported in us-east-1, ap-northeast-2, eu-west-1, so this Feature/Offerings should be deployed ONLY in these regions
'''
    ...
    try:
        ...
        if (event['RequestType'] in ['Create','Update']) and ('ServiceToken' in event):
            try:
                ...
                dynamodb_response = dynamodb_client.get_item(TableName='AccountFeatureDefinitions',Key={"Feature":{'S':"AWSLookoutForEquipmentMonitoring"}}, ConsistentRead=True)
                ...
                if report:
                    text="AWS Lookout For Equipment Monitoring, create CW alarms report:\n\n"
                    text+="Sucess: "+str(success_count)+"\nFailed: "+str(failure_count)+"\n\nReport Information:\n\n"
                    print("Alarms creation report - ",report)
                    subject= 'DXCMS AWS Lookout For Equipment Monitoring, CW Alarms report. acc-'+ str(acc_id) + ' region ' + region
