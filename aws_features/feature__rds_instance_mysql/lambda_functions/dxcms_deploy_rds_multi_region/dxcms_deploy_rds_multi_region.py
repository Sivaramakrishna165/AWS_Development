...
def create_stack(client,stack_name,template_url,parameters):
    try:
        response = client.create_stack(StackName=stack_name,TemplateURL=template_url,Parameters=parameters,Tags=[{'Key': "Owner",'Value': "DXC"}], Capabilities=['CAPABILITY_IAM'])
...
def lambda_handler(event, context):
    try:
        ...
        if event['RequestType'] in ['Delete']:
            ...
            s3_template_key=os.environ['S3_KEY'].replace("lambda_functions","resource_deployment/RDSInstanceMysqlMultiRegion.yaml")
                ...
                if (event['RequestType'] in ['Update']):
                    ...
                    if str(describe_stack(secondary_cft_client,ssm_stack_name)).lower()=="stack present":
                        ...
                        if 'ResourceProperties' in event:
                            ...
                            ssm_param="/DXC/RDS-MYSQL/PRIMARY/TABLE/ITEMS-"+ssm_stack_name.split("-")[5]
            ...
            stack_name='Feature-RDSInstanceMysql-Multi-region-stack-{}'.format(str(uuid4()))
            ssm_param=event['ResourceProperties']["ssm_param"]
