...
Lambda function does the below
    Reads the Alarms from 'AWSGlueMonitoring' item available in DDB - AccountFeatureDefinition
    And creates CW alarms for AWS Glue service
'''
    ...
    try:
        ...
        if (event['RequestType'] in ['Create','Update']) and ('ServiceToken' in event):
            try:
                ...
                dynamodb_response = dynamodb_client.get_item(TableName='AccountFeatureDefinitions',Key={"Feature":{'S':"AWSGlueMonitoring"}}, ConsistentRead=True)
                ...
                for alarm in alarms:
                    try:
                        ...
                        alarm_Dimensions='-'.join([dimlist['Type'],dimlist['JobRunId'],dimlist['JobName']])
                ...
                if report:
                    text="AWS Glue Monitoring, create CW alarms report:\n\n"
                    text+="Sucess: "+str(success_count)+"\nFailed: "+str(failure_count)+"\n\nReport Information:\n\n"
                    print("Alarms creation report - ",report)
                    subject= 'DXCMS AWS Glue Monitoring, CW Alarms CREATE report. account - '+ str(acc_id) + ' region ' + region
