...
Lambda function does the below
    Reads the Alarms from 'AWSAthenaMonitoring' item available in DDB - AccountFeatureDefinition
    And creates CW alarms for AWS Athena service
'''    
import os
import boto3
import json
    ...
    try:
        ...
        if (event['RequestType'] in ['Create','Update']) and ('ServiceToken' in event):
            try:
                ...
                dynamodb_response = dynamodb_client.get_item(TableName='AccountFeatureDefinitions',Key={"Feature":{'S':"AWSAthenaMonitoring"}}, ConsistentRead=True)
                ...
                for alarm in alarms:
                    try:
                        ...
                        alarm_Dimensions='-'.join([dimlist['WorkGroup'],dimlist['QueryState'],dimlist['QueryType']])
                ...
                if report:
                    text="AWS Athena Monitoring, create CW alarms report:\n\n"
                    text+="Sucess: "+str(success_count)+"\nFailed: "+str(failure_count)+"\n\nReport Information:\n\n"
                    print("Alarms creation report - ",report)
                    subject= 'DXCMS AWS Athena Monitoring, CW Alarms CREATE report. account - '+ str(acc_id) + ' region ' + region
