...
def lambda_handler(event, context):
    ...
    try:
        ...
        try:    
            ...
            error = traceback.format_exc()
            raise Exception(f"Error while reading AccountId. Incident in Service Now is NOT created for {instance_id} crowdstrike issue.")
        
        arn_prefix = "arn:aws:states:" + region + ":" + AccountId + ":stateMachine:"

        short_message = f"AWS_SH_CS {AccountId} {region} | CrowdStrike/Falcon Agent is NOT installed for {instance_id}."

        long_message = f"SelfHeal CrowdStrike Step Function caught error. CS agent failure not resolved. Details : {str(event)}."
        ...
        if error:
            raise Exception(f"Error while triggering dxcms_sh_sfn_notification step function. Incident in Service Now is NOT created for {instance_id} crowdstrike issue.")
