"""
    This Lambda function is used to check Falcon_Agent_Status wheather Falcon agent is installed ot not.
    If falcon agent is installed shows as installed.
    If falcon agent is not_installed shows as not_installed.
...
In dxcms_sh_cs_sfn_diagnosis state machine FetchFalconAgentDetails,gets executed after CheckCLIStatus
On successful check, next state Parallel State(SelfHealDiagnosisResult,StateMachineInfo) is called.
On FAILURE, next State FetchFalconAgentDetailsError and then NotifyForLambaFunctionFailure.
...
def falcon_agent_check(instance_id,platform):
            ...
            try:
                ...
                if platform == 'Windows':
                    ...
                    if output['StandardOutputContent'] !="":
                        ...
                        if output_line.startswith('Active: active (running)'):  
    ...
    try:
        ...
        if not ssm_error:
            if ssm_ping_status == 'Present':
                ...
                status, error_status = falcon_agent_check(instance_id,platform)
                if not error_status:
            ...
            print ("Error while checking ssm connection")
